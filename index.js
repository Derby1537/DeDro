...
ipcMain.handle('connect-to-drone', async (_event, drone) => {
    ...
    try {
        ...
        for(let i = 0; i < services.length; i++) {
            if(services[i].uuid === HW_SERV_UUID) {
                ...
                for(let j = 0; j < characteristics.length; j++) {
                    if(characteristics[j].uuid === ENVIRONMENTAL_CHAR_UUID) {
                        droneBluetoothPeripheral.ENVIRONMENTAL_CHAR_UUID = characteristics[j];
                    ...
                    if(characteristics[j].uuid === ACC_GYRO_MAG_CHAR_UUID) {
                        droneBluetoothPeripheral.ACC_GYRO_MAG_CHAR_UUID = characteristics[j];
                        await characteristics[j].subscribeAsync();
                        characteristics[j].on('data', (data, _isNotification) => {
                            if (win) {
            ...
            //if(services[i].uuid === CONSOLE_SERV_UUID) {
            //    const characteristics = await services[i].discoverCharacteristicsAsync([
            //        TERM_CHAR_UUID, 
            //        STDERR_CHAR_UUID
            //    ]);
            ...
            //            await characteristics[j].subscribeAsync();
            //            characteristics[j].on('data', (data, isNotification) => {
            //                socket.emit('TERM_CHAR', data.toString());
            //            })
            //        }
            ...
            //            await characteristics[j].subscribeAsync();
            //            characteristics[j].on('data', (data, isNotification) => {
            //                socket.emit('STDERR_CHAR', data.toString());
            //            })
            //        }
            //    }
            //}
            //if(services[i].uuid === CONFIG_SERV_UUID) {
            //    console.log("Caratteristiche config");
            //    const characteristics = await services[i].discoverCharacteristicsAsync([
            //        CONFIG_CHAR_UUID
            //    ]);
            //    for(let j = 0; j < characteristics.length; j++) {
            //        console.log(" -> " + characteristics[j].uuid)
            //        if(characteristics[j].uuid === CONFIG_CHAR_UUID) {
            //            droneBluetoothPeripheral.CONFIG_CHAR_UUID = characteristics[j];
            //        }
            //    }
            //}
        }

        if(!droneBluetoothPeripheral.MAX_CHAR_UUID) { /*Comment when you want to connect to anything*/
        ...
        peripheral.on('disconnect', () => {
            droneBluetoothPeripheral = null;
            //socket.emit('drone-status', false);
            if (win) {
